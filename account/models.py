...
class CustomAccountManager(BaseUserManager):
    ...
    def create_user(self, email, username, password, **other_fields):
        ...
        if not email: # if they haven't provided an email
            raise ValueError(
                _("You must provide an email address")
            )

        email = self.normalize_email(email) # check if email has correct format
        user = self.model(email=email, user_name=username, **other_fields)
